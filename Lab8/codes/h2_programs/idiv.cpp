...
#define IDIV_PORT   12503          // The port clients will be connecting to
...
string processData(string data, struct sockaddr *client) {
    ...
    // Get the IP address of the client (H1)
    string clientIP = TCPServer::getIP(client);
...
int main() {
    ...
    // Creating a TCP server (for h1 -> h2)
    TCPServer idivServer(IDIV_PORT, status, DOMAIN);
    if (status == -1) exit(EXIT_FAILURE);

    // Create a UDP client (for h2 -> h3)
    UDPClient idivClient(H3_IP, H3_PORT, status, DOMAIN);
    if (status == -1) exit(EXIT_FAILURE);

    // Start listening on the server (h2)
    if (idivServer.startListening(BACKLOG) == -1) exit(EXIT_FAILURE);
    ...
    ll newFd = 0;  // waitpid() might overwrite errno, so we save and restore it by providing our handler

    while (1) {
        // Accept connections from the client (h1)
        if ((newFd = idivServer.acceptConnection(&clientAddr)) == -1) {
            continue;
        }

        if (!fork()) {                      // this is the child process
            close(idivServer.getSockFD());  // child doesn't need the listener

            // Read data from the client (h1)
            string data = "";
            try {
                data = idivServer.receiveData(newFd);
            ...
            if (idivClient.sendData(processData(data, (struct sockaddr *)&clientAddr)) == -1) exit(EXIT_FAILURE);

            // Done with these socket file descriptors
            close(newFd);
            exit(EXIT_SUCCESS);
        }
