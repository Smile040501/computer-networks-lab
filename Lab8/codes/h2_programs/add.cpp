...
#define ADD_PORT    12500          // The port clients will be connecting to
...
string processData(const string &data, struct sockaddr *client) {
...
int main() {
    ...
    TCPServer addServer(ADD_PORT, status, DOMAIN);
    if (status == -1) exit(EXIT_FAILURE);

    // Create a UDP client (for h2 -> h3)
    UDPClient addClient(H3_IP, H3_PORT, status, DOMAIN);
    if (status == -1) exit(EXIT_FAILURE);

    // Start listening on the server (h2)
    if (addServer.startListening(BACKLOG) == -1) exit(EXIT_FAILURE);
    ...
    while (1) {
        ...
        if ((newFd = addServer.acceptConnection(&clientAddr)) == -1) {
            continue;
        }

        if (!fork()) {                     // this is the child process
            close(addServer.getSockFD());  // child doesn't need the listener

            // Read data from the client (h1)
            string data = "";
            try {
                data = addServer.receiveData(newFd);
            ...
            if (addClient.sendData(processData(data, (struct sockaddr *)&clientAddr)) == -1) exit(EXIT_FAILURE);
