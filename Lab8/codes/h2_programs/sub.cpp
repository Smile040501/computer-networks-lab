...
int main() {
    ...
    TCPServer subServer(SUB_PORT, status, DOMAIN);
    ...
    if (subServer.startListening(BACKLOG) == -1) exit(EXIT_FAILURE);
    ...
    while (1) {
        ...
        if ((newFd = subServer.acceptConnection(&clientAddr)) == -1) {
            continue;
        }

        if (!fork()) {                     // this is the child process
            close(subServer.getSockFD());  // child doesn't need the listener

            // Read data from the client (h1)
            string data = "";
            try {
                data = subServer.receiveData(newFd);
            ...
            if (subClient.sendData(processData(data, (struct sockaddr *)&clientAddr)) == -1) exit(EXIT_FAILURE);
