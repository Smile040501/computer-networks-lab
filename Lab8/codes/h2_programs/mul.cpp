...
#define MUL_PORT    12502          // The port clients will be connecting to
...
int main() {
    ...
    TCPServer mulServer(MUL_PORT, status, DOMAIN);
    if (status == -1) exit(EXIT_FAILURE);

    // Create a UDP client (for h2 -> h3)
    UDPClient mulClient(H3_IP, H3_PORT, status, DOMAIN);
    if (status == -1) exit(EXIT_FAILURE);

    // Start listening on the server (h2)
    if (mulServer.startListening(BACKLOG) == -1) exit(EXIT_FAILURE);
    ...
    while (1) {
        ...
        if ((newFd = mulServer.acceptConnection(&clientAddr)) == -1) {
            continue;
        }

        if (!fork()) {                     // this is the child process
            close(mulServer.getSockFD());  // child doesn't need the listener

            // Read data from the client (h1)
            string data = "";
            try {
                data = mulServer.receiveData(newFd);
            ...
            if (mulClient.sendData(processData(data, (struct sockaddr *)&clientAddr)) == -1) exit(EXIT_FAILURE);
