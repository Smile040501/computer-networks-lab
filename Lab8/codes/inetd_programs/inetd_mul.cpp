...
#define ARGS_STR "./inet_mul <clientIP>"
...
int main(int argc, char const *argv[]) {
    ...
    TCPServer mulServer(STDIN_FILENO);
    ...
    string data = "";
    try {
        data = mulServer.receiveData(STDIN_FILENO);
    ...
    if (mulClient.sendData(processData(data, clientIP)) == -1) exit(EXIT_FAILURE);
