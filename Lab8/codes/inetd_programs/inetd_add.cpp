...
#define ARGS_STR "./inet_add <clientIP>"
...
int main(int argc, char const *argv[]) {
    ...
    TCPServer addServer(STDIN_FILENO);
    ...
    string data = "";
    try {
        data = addServer.receiveData(STDIN_FILENO);
    ...
    if (addClient.sendData(processData(data, clientIP)) == -1) exit(EXIT_FAILURE);
