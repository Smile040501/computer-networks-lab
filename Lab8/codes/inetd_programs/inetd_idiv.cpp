...
#define ARGS_STR "./inet_idiv <clientIP>"
...
int main(int argc, char const *argv[]) {
    ...
    TCPServer idivServer(STDIN_FILENO);
    ...
    string data = "";
    try {
        data = idivServer.receiveData(STDIN_FILENO);
    ...
    if (idivClient.sendData(processData(data, clientIP)) == -1) exit(EXIT_FAILURE);
