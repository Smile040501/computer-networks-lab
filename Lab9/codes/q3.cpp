...
using packId = unsigned short;  // Datatype for number of bits in the packet Identifier
...
// Function to get the vector of all the valid packets
vector<IPPacket*> getValidPackets() {
    vector<IPPacket*> result;
    ...
    for (auto const& dirEntry : fs::directory_iterator(ipfrags)) {
        ...
        if (validateChecksum(packet)) {
            ...
            // Free up the space allocated for the invalid packet
            delete packet;
        }
...
// Function to validate that a received message contains all the IP fragments
// and the More Fragment bit of the last fragment is 0
bool validateMessage(const vector<IPPacket*>& frags) {
    // The current offset received as per the length of the data received so far
    ull currOffset = 0;

    const IPPacket* lastFrag = nullptr;

    // Iterate over all the fragments
    for (const auto& f : frags) {
        // The data length in the packet
        ull dataLen = f->getDataLen();

        // Skip if the data length is zero
        if (dataLen != 0) {
            // If the More Fragment flag of the intermediate packets is not 1
            if (lastFrag != nullptr && lastFrag->getFlags().second.second != 1) {
                // It should not happen
                return false;
            }

            // If the offset of the current fragment is not as expected
            if (currOffset != f->getFragOffset()) {
                // Not all the fragments have arrived
                return false;
            }

            // Next offset will be at the distance of number of bytes of the data length in the current packet
            currOffset += dataLen / 8;

            // Update the last IP packet fragment in the message
            lastFrag = f;
        }
    }

    // If the More Fragment flag of the last fragment is not 0
    if (lastFrag->getFlags().second.second != 0) {
        // Last Fragment has not arrived
        return false;
    }
...
int main() {
    ...
    vector<IPPacket*> validPackets = getValidPackets();

    // Messages having the same packet identifier
    map<packId, vector<IPPacket*>> msgs;

    // Group messages having the same packet identifier
    for (const auto& p : validPackets) {
        packId id = p->getID();
        if (msgs.find(id) == msgs.end()) {
            // If packet ID is not present, create a new one
            msgs[id] = vector<IPPacket*>();
        }
        msgs[id].emplace_back(p);
    }

    std::cout << "Number of messages: " << msgs.size() << "\n\n";

    // Constructing all the messages
    for (const auto& p : msgs) {
        ull packetSize = 0;
        vector<IPPacket*> frags = p.second;  // The fragments for a message

        // Sort the fragments in ascending order of fragment offset
        sort(frags.begin(), frags.end(), [](const IPPacket* x, const IPPacket* y) {
            return x->getFragOffset() < y->getFragOffset();
        });

        // Check if the message received is valid
        if (!validateMessage(frags)) {
            std::cerr << "Invalid message received with ID: " << p.first << '\n';
            continue;
        }
        ...
        string msg = "";
        for (const auto& f : frags) {
            // Append individual data from all the fragments
            msg += f->getData();

            // Take the maximum header length as the header length for the original packet
            packetSize = max(packetSize, f->getHeaderLen() * 4);
        }

        // Add the final data length to the original packet size
        packetSize += msg.length();

        std::cout << std::left << setw(20) << "Got Packet with ID"
                  << " : " << p.first << '\n'
                  << std::left << setw(20) << "Packet Size"
                  << " : " << packetSize << '\n'
                  << std::left << setw(20) << "Number of Fragments"
                  << " : " << p.second.size() << '\n'
                  << std::left << setw(20) << "Message Data"
                  << " : " << msg << '\n'
                  << std::left << setw(20) << "Message Length"
                  << " : " << msg.length() << "\n\n";
    }

    // Freeing up the space allocated for the valid IP packets
    for (const auto& p : validPackets) {
        delete p;
    }
